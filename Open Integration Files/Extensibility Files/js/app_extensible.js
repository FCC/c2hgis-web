...
var table_name = 'Connect2Health_2023_ui_';
var table_name_dl = 'Connect2Health_2023_';
...
function createMap() {
    ...
    map.attributionControl.addAttribution('<a href="http://fcc.gov">FCC<span class="sr-only">Home Page</span></a>&nbsp|&nbsp<a href="https://www.fcc.gov/about-fcc/fcc-initiatives/connect2healthfcc">Connect2Health</a>');
    ...
    $('.leaflet-control-fullscreen-button').html('<span class="sr-only">View Full Screen</span>');
...
function updateSlider(slider, min, max, applyFilter) {
    ...
    if (slider === 'broadband' || slider === 'health') {
        ...
        if (column.includes('pct')) {
            ...
            var endLabel = column !== 'opioid_prescribing_rate' ? ' overdose deaths/100 pop.)' : ' prescription rates/100k pop.)';
    ...
    var extFilter = getExtPopFilter();

    // Open integration
    if (cur_tab === 'insights' && extFilter) {
        applyExtFilter(slider, filter, extFilter);
    } else {
        redoMap(slider, filter, zindex);
    }
}
...
function generateMenu() {
    $('.layer-switch').find('li').removeClass('active').find('a').removeAttr('aria-selected');    
    $('#' + cur_tab).parent('li').addClass('active').find('a').attr('aria-selected', true);
