...
function getData(zoomCenter) {
    ...
    var data_url = geo_host + '/' + geo_space + '/wfs?service=WFS&version=1.0.0&request=GetFeature&typeName=' + geo_space + ':' + table_name + data_type + '&maxFeatures=1&outputFormat=application/json&cql_filter=contains(geom,%20POINT(' + geo_lng + ' ' + geo_lat + '))';
...
function setSlider(slider, values) {
    ...
    var nationAvg = ref === 'opioid' ? insight_ly[ref][dropdown].nationAvg : null;

    var tooltip = insight_ly[ref][dropdown].tooltip;

    // Add national average to tooltip
    if (ref === 'opioid') tooltip += ' 2018 National Average: ' + nationAvg;
    ...
    if (slider === 'broadband' || slider === 'health') {
        ...
        if (filterType === 'trends') {
            ...
            $('#label-nationalAverage').text('NOTE: 1x = ' + nationAvg + label + '/100k pop. (2018 Natl. Avg.)');
...
function updateSlider(slider, min, max, applyFilter) {
    ...
    if (slider === 'broadband' || slider === 'health') {
        ...
        filterType = $('#select-in-opioid').find(':selected').data('filtertype');
        ...
        filterType = $('#select-in-' + slider).find(':selected').data('filtertype');
        ...
        if (filterType === 'trends') {
            ...
            var nationAvg = insight_ly[ref][dropdown].nationAvg;
...
function updateOpioidLegend() {
    ...
    if (opioid_ly[opioid_type]) {
        ...
        var nationAvg = insight_ly['opioid'][opioid_type].nationAvg;
        // nationAvg = nationAvg ? Math.round(nationAvg * 10) / 10 : null;
        var opioid_tooltip = insight_ly['opioid'][opioid_type].tooltip + ' 2018 National Average: ' + nationAvg;
...
function updateStats() {
    ...
    if (geo_prop[insight_ly.broadband[broadband_sel].column] !== null) {
        ...
        // if (broadband_sel === 'in_bb_dl_speed' && geo_prop.geography_type === 'nation') {
        //     broadband_stat_value = bb_speed_tiers['7'].range + ' mbps';
        // }
        // if (broadband_sel === 'in_bb_ul_speed' && geo_prop.geography_type === 'nation') {
        //     broadband_stat_value = bb_speed_tiers['2'].range + ' mbps';
        // }
    } else {
    ...
    if (geo_prop[insight_ly.opioid[opioid_sel].column] !== null) {
        if (insight_ly.opioid[opioid_sel].column.includes('pct')) {   // pct change
            opioid_stat_value = geo_prop[insight_ly.opioid[opioid_sel].column] + '%';
        } else {
            opioid_stat_value = geo_prop[insight_ly.opioid[opioid_sel].column] + ' per 100,000';
        }
        $('#tooltip-overview-na').hide();
    } else {
        // NULLS
        if (insight_ly.opioid[opioid_sel].column.includes('pct')) {
            opioid_stat_value = 'Unavailable';
        } else {
            opioid_stat_value = 'Not reported';
        }
    ...
    // if (geo_prop.geography_type === 'nation' && geo_prop[insight_ly.opioid[opioid_sel].column] !== null) {
    //     opioid_stat_value = insight_ly.opioid[opioid_sel].nationAvg;
    //     if (insight_ly.opioid[opioid_sel].column.includes('pct')) opioid_stat_value += '%';
    //     else opioid_stat_value += ' per 100,000'
    // }
    ...
    if (curr_health_measure_type === 'opioid') {
        ...
        if (opioidSel !== 'none') {
            ...
            if (geo_prop[col] !== null) {
                // if (geo_prop.geography_type === 'nation') {
                //     opioidStatVal = insight_ly.opioid[opioidSel].nationAvg;
                // } else {
                    opioidStatVal = geo_prop[insight_ly.opioid[opioidSel].column]
                // }
                opioidStatVal = formatStatAppend(opioidStatVal, null, append);
            } else {
                $('#tooltip-bb-na').show();
                if (col.includes('pct')) opioidStatVal = 'Unavailable';
                else opioidStatVal = 'Not reported';
            }
            $('.geog-opioid-stat').show();
            $('.geog-opioid-stat-none').hide();
        } else {
    ...
    // Health tab ======================================================================================================
    // $('.geog-all-drugs-deaths').text(defineOpioidNull('alldrugs_age_adj_mortality_rate', geo_prop.geography_type === 'nation' ? insight_ly.opioid.in_alldrugs_age_adj_mortality_rate.nationAvg : geo_prop.alldrugs_age_adj_mortality_rate));
    // $('.geog-all-drugs-death-trends').text(defineOpioidNull('alldrugs_age_adj_mortality_rate_pct_change', geo_prop.geography_type === 'nation' ? insight_ly.opioid.in_alldrugs_age_adj_mortality_rate_pct_change.nationAvg : geo_prop.alldrugs_age_adj_mortality_rate_pct_change));
    // $('.geog-all-opioid-deaths').text(defineOpioidNull('anyopioids_age_adj_mortality_rate', geo_prop.geography_type === 'nation' ? insight_ly.opioid.in_anyopioids_age_adj_mortality_rate.nationAvg : geo_prop.anyopioids_age_adj_mortality_rate));
    // $('.geog-all-opioid-death-trends').text(defineOpioidNull('anyopioids_age_adj_mortality_rate_pct_change', geo_prop.geography_type === 'nation' ? insight_ly.opioid.in_anyopioids_age_adj_mortality_rate_pct_change.nationAvg : geo_prop.anyopioids_age_adj_mortality_rate_pct_change));
    // $('.geog-rx-opioid-deaths').text(defineOpioidNull('prescriptionopioids_age_adj_mortality_rate', geo_prop.geography_type === 'nation' ? insight_ly.opioid.in_prescriptionopioids_age_adj_mortality_rate.nationAvg : geo_prop.prescriptionopioids_age_adj_mortality_rate));
    // $('.geog-rx-opioid-death-trends').text(defineOpioidNull('prescriptionopioids_age_adj_mortality_rate_pct_change', geo_prop.geography_type === 'nation' ? insight_ly.opioid.in_prescriptionopioids_age_adj_mortality_rate_pct_change.nationAvg : geo_prop.prescriptionopioids_age_adj_mortality_rate_pct_change));
    // $('.geog-synthetic-opioid-deaths').text(defineOpioidNull('syntheticopioids_age_adj_mortality_rate', geo_prop.geography_type === 'nation' ? insight_ly.opioid.in_syntheticopioids_age_adj_mortality_rate.nationAvg : geo_prop.syntheticopioids_age_adj_mortality_rate));
    // $('.geog-synthetic-opioid-death-trends').text(defineOpioidNull('syntheticopioids_age_adj_mortality_rate_pct_change', geo_prop.geography_type === 'nation' ? insight_ly.opioid.in_syntheticopioids_age_adj_mortality_rate_pct_change.nationAvg : geo_prop.syntheticopioids_age_adj_mortality_rate_pct_change));
    // $('.geog-heroin-deaths').text(defineOpioidNull('heroin_age_adj_mortality_rate', geo_prop.geography_type === 'nation' ? insight_ly.opioid.in_heroin_age_adj_mortality_rate.nationAvg : geo_prop.heroin_age_adj_mortality_rate));
    // $('.geog-heroin-death-trends').text(defineOpioidNull('heroin_age_adj_mortality_rate_pct_change', geo_prop.geography_type === 'nation' ? insight_ly.opioid.in_heroin_age_adj_mortality_rate_pct_change.nationAvg : geo_prop.heroin_age_adj_mortality_rate_pct_change));
    // $('.geog-rx-rates').text(defineOpioidNull('opioid_prescribing_rate', geo_prop.geography_type === 'nation' ? insight_ly.opioid.in_opioid_prescribing_rate.nationAvg : geo_prop.opioid_prescribing_rate));
    // $('.geog-rx-trends').text(defineOpioidNull('opioid_prescribing_rate_pct_change', geo_prop.geography_type === 'nation' ? insight_ly.opioid.in_opioid_prescribing_rate_pct_change.nationAvg : geo_prop.opioid_prescribing_rate_pct_change));
